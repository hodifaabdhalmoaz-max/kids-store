  ...
  <div class="category-header">
    <div class="container position-relative">
      <div class="breadcrumb-dashboard mx-auto mb-3">
        ...
        <span style="color: #1a202c; font-weight: 700;">نتائج البحث</span>
      </div>
      @if($search_query)
        <h1>نتائج البحث عن: "{{ $search_query }}"</h1>
      @else
        <h1>جميع المنتجات</h1>
      @endif
    
    <div class="search-bar-wrapper d-flex align-items-center">
      <x-search-bar class="w-100" placeholder="ابحث الآن..." />
    </div>
    </div>
  </div>
  ...
  <div class="container">
    ...
    <div class="d-flex align-items-center mb-4">
      ...
      <div class="filter-tabs-container flex-grow-1 ms-2" style="margin-top: 0; padding: 0;">
        <a href="{{ route('shop.search', ['search' => $search_query, 'tab' => 'all']) }}" class="filter-tab {{ $current_tab == 'all' ? 'active' : '' }}">
          ({{ $products->total() }}) الكل
        </a>
        @foreach($tabs as $tab)
        <a href="{{ route('shop.search', ['search' => $search_query, 'tab' => $tab]) }}" class="filter-tab {{ $current_tab == $tab ? 'active' : '' }}">
          {{ $tab }}
        </a>
        @endforeach
      </div>
    ...
    <div class="products-grid row row-cols-2 row-cols-md-3 row-cols-lg-4 g-3 g-md-4" id="products-grid">
      ...
      <div class="col-12 text-center py-5">
        <h5 class="text-muted">لا توجد منتجات مطابقة لبحثك.</h5>
      </div>
      @endforelse
    </div>
...
<x-sort-modal
  :action="route('shop.search')"
  :extra-inputs="['search' => request('search'), 'tab' => request('tab', 'all')]"
/>

<x-filter-modal
  :action="route('shop.search')"
  :reset-url="route('shop.search', ['search' => request('search')])"
  :brands="$brands"
  :colors="$colors"
  :sizes="$sizes"
  :extra-inputs="['search' => request('search'), 'tab' => request('tab', 'all')]"
