      ...
      <ul class="nav nav-tabs mb-5" id="login_register" role="tablist">
        <li class="nav-item" role="presentation">
          <a class="nav-link nav-link_underscore active" id="login-tab" data-bs-toggle="tab" href="#tab-item-login"
            role="tab" aria-controls="tab-item-login" aria-selected="true">تسجيل الدخول</a>
        </li>
      </ul>
        ...
        <div class="tab-pane fade show active" id="tab-item-login" role="tabpanel" aria-labelledby="login-tab">
          <div class="login-form">
            <form method="POST" action="{{route('login')}}" name="login-form" class="needs-validation" novalidate="">
              ...
              <button class="btn btn-primary w-100 text-uppercase" type="submit">دخول</button>

              <div class="customer-option mt-4 text-center">
                <span class="text-secondary">ليس لديك حساب؟</span>
                <a href="{{route('register')}}" class="btn-text js-show-register">إنشاء حساب جديد</a>
              </div>
            </form>
          </div>
        </div>
